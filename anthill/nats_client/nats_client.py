...
class NATSClient:
    ...
    def check_connection(self):
        if self.connector.client.check_connection:
            ...
            self.log.warning(
                f"NATS connection status: {self.connector.client.check_connection}"
            )
    ...
    def publish_request(
        self, message, topic: str, timeout: int = 10, unpack: bool = True
    ):
        return self.connector.publish_request(
            message, topic, timeout=timeout, unpack=unpack
        )

    def publish_request_with_reply_to_another_topic(
        self, message, topic: str, reply_to: str = None
    ):
        self.connector.publish_request_with_reply_to_another_topic(
    ...
    async def aio_publish_request(
        ...
        return await self.connector.aio_publish_request(
            message, topic, timeout, unpack=unpack
        )

    async def aio_publish_request_with_reply_to_another_topic(
        self, message, topic: str, reply_to: str = None
    ):
        await self.connector.aio_publish_request_with_reply_to_another_topic(
            message, topic, reply_to
        )

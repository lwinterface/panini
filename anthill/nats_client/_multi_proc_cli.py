...
LISTEN_FOR_NEW_SUBSCRIPTION_TOPIC = (
    ...
    else str(uuid.uuid4())[10:] + "__new_subscribtion "
)
...
class _MultiProcNATSClient(object):
    ...
    async def aio_publish(
        self, message, topic: str, reply_to: str = None, force: bool = None
    ):
        self.publish(message, topic)
...
class _ListenerProc:
    def __init__(
        ...
        self.loop.create_task(
            self.listen_for_new_subscribtion(LISTEN_FOR_NEW_SUBSCRIPTION_TOPIC)
        )

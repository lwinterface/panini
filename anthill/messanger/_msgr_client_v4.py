...
log = Logger(name='_MessengerClient_V4').log
isr_log = InterServicesRequestLogger(name='InterServicesRequest_V4').isr_log
...
class _MessengerClient:
        ...
        if self.client.is_connected:
            ...
            for topic, callbacks in listen_topics_callbacks.items():
                for callback in callbacks:
                    ...
                    await self.client.subscribe(topic, queue=self.queue, cb=wrapped_callback, pending_bytes_limit=65536 * 1024 * 10)
                log(f'{os.environ["CLIENT_ID"]} subscribed to topic: {topic}')
        return
                    ...
                    async def coro_callback_with_reply(subject, data, reply_to, isr_id):
                        try:
                            ...
                            if reply_to:
                                ...
                                # isr_log(f"4SENDING RESPONSE msg: isr_id: {isr_id} reply_to:{reply_to}({type(reply_to)}) {data}, {subject}, reply: {str(reply)[:200]}")
                                await cli.aio_publish(reply, reply_to)
                        except Exception as e:
                            if not 'reply' in locals():
                                reply = ""
                            raise Exception(f"4RESPONSE callback_when_future_finished ERROR: {str(e)}, reply if exist: {reply}")
                    try:
                        ...
                        raise Exception(f"4RESPONSE callback ERROR: {str(e)}")
                else:
    ...
    def publish_request_reply_to_another_topic(self, message, topic, reply_to=None):
        asyncio.ensure_future(self.aio_publish_request_reply_to_another_topic(message, topic, reply_to))
    ...
    async def aio_publish(self, message, topic, force=False):
        ...
        if not force:
            ...
            await self._aio_force_publish(topic, message)

    async def _aio_force_publish(self, topic, message):
        async def empty_cb(*args, **kwargs):
            pass
        await self.client.request(topic, message, cb=empty_cb)
...
if __name__ == "__main__":
    ...
    async def amsg_req_generator_v2(cli):
        ...
        print('amsg_generator started')
        async def request(i):
            result = await cli.aio_publish_request({'data':f' =======>>>>>>some message number {str(i)}'}, 'topic2.wqe', timeout=60)
            print(result)
            return result
        start = datetime.datetime.now().timestamp()

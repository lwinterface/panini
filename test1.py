...
class Symbol(Serializer):
    code = Field(type=str)
    name = Field(type=str, default=None, null=True)


class Ticker(Serializer):
    symbol = Field(type=Symbol)
    open = Field(type=float)
    high = Field(type=float)
    low = Field(type=float)
    close = Field(type=float)
    last = Field(type=float)
    volume = Field(type=float)
    timestamp = Field(type=float, default=datetime.datetime.now().timestamp())
    custom1 = Field(type=float, default=None, null=True)
    list_of_something = Field(type=list, default=[])



if __name__ == '__main__':
    message = {
        'symbol':{'code':'BTC/USDT', 'name':'namenamename'},
        'open':324.324,
        'high':234.234,
        'low':123.123,
        'close':456.456,
        ...
        'custom1': 234.435,
        'list_of_something':[1,2,3,4,5],
    }

    print(Ticker.validate_message(message))
